...
export declare function deferAgentContextMessage(agent: ActiveAgent, content: string, messageId?: string): void;
/** Atomically reserve deferred context for one substantive delivery. */
export declare function takeDeferredAgentContext(agent: ActiveAgent): DeferredContextMessage[];
/** Restore a failed delivery ahead of status messages that arrived meanwhile. */
export declare function restoreDeferredAgentContext(agent: ActiveAgent, messages: readonly DeferredContextMessage[]): void;
/** Attach one reserved status-context snapshot to a substantive delivery. */
export declare function messageWithDeferredAgentContext(context: readonly DeferredContextMessage[], message: string): string;
/** Empty successful follow-up turns update lifecycle state but need no model notification. */
export declare function shouldPublishAdditionalTurn(result: SingleResult, knownWarnings?: Set<string>): boolean;
...
export declare const TEAMMATE_WAIT_DESCRIPTION = "Wait once for a teammate result by correlation ID (a task name is a compatibility alias), or provide waitMs for a fixed delay. Named waits default to a bounded 600000 ms (10 minutes) timeout and settle on result-ready (not terminal lifecycle); they are the single-target convenience form of observe action=\"wait\" \u2014 use observe with until=\"completed\" to wait for full termination. Agent waits replace repeated teammate-watch calls.";
