...
export interface ClassifyDomain<D extends string, I = unknown> {
  ...
  decide(answers: JevAnswers): { label: D; confidence: number; probabilities?: Record<string, number> } | undefined;
  /** Fail-safe result for transport/parse/timeout failures. */
  fallback(reason: string): { label: D; confidence: number };
}
...
export interface ClassifyShadowRecord {
  ...
  at: string;
  state: string;
  rule: { label: string; terminal: boolean } | null;
  jev?: { label: string; confidence: number; model?: string };
  agree?: boolean;
  error?: string;
}
